...
export const commonSchemas = {
  ...
  updateOrderStatus: z.object({
    status: z.enum(
      ["waiting", "in_production", "paused", "completed", "cancelled"],
      {
  ...
  smsMessage: z
    ...
    .refine(
      (data) =>
        data.phone_number || (data.recipients && data.recipients.length > 0),
      { message: "يجب تحديد رقم هاتف واحد على الأقل أو قائمة مستلمين" },
    ),
};

// Session validation middleware
// NOTE: Duplicate requireAuth/requireAdmin definitions were removed (Phase 1 cleanup).
// The canonical implementations live in `server/middleware/auth.ts` and are
// permission-aware via the role-based ACL. Importers must use that module.
