...
function sectionTitle(text: string): Paragraph {
  return new Paragraph({
    children: [
      txt(`  ${text}`, { bold: true, size: 24, color: COLORS.headerText }),
    ],
...
async function getCompanyLogoBufferForDocx(
  fallbackPath: string,
): Promise<Buffer> {
  try {
        ...
        headers: {
          default: new Header({
            children: [
              new Paragraph({
                children: [
                  txt("مصنع الأكياس البلاستيكية الحديثة", {
                    size: 14,
        ...
        footers: {
          default: new Footer({
            children: [
              new Paragraph({
                children: [
                  txt("Modern Plastic Bag Factory | ", {
                    size: 14,
                    color: COLORS.textLight,
                    rtl: false,
                  }),
                  txt("هاتف: {{company_phone}} | ", {
                    size: 14,
                    color: COLORS.textLight,
                  }),
                  txt("بريد: {{company_email}}", {
                    size: 14,
                    color: COLORS.textLight,
                  }),
                ],
                  ...
                  new TableCell({
                    children: [
                      ...
                      new Paragraph({
                        children: [
                          txt(
                            "سجل تجاري: {{company_cr}} | رقم ضريبي: {{company_vat}}",
...
function generateItemRows(maxItems: number): TableRow[] {
  ...
  for (let i = 1; i <= maxItems; i++) {
    ...
    rows.push(
      new TableRow({
        children: [
          mergeCell(`${prefix}number`, {
            width: 5,
            align: AlignmentType.CENTER,
          }),
          mergeCell(`${prefix}name`, { width: 30, bold: true }),
          mergeCell(`${prefix}size`, {
            width: 20,
            align: AlignmentType.CENTER,
          }),
          mergeCell(`${prefix}quantity`, {
            width: 10,
            align: AlignmentType.CENTER,
          }),
          mergeCell(`${prefix}unit`, { width: 8, align: AlignmentType.CENTER }),
          mergeCell(`${prefix}unit_price`, {
            width: 12,
            align: AlignmentType.CENTER,
          }),
          mergeCell(`${prefix}total`, {
            width: 15,
            align: AlignmentType.CENTER,
            bold: true,
            color: COLORS.primary,
