...
export default function MaterialMixing() {
  ...
  const handleSave = () => {
    if (!productionOrderId) {
      toast({
        title: t("common.error"),
        description: t("mixing.messages.selectProductionOrder"),
        variant: "destructive",
      });
      return;
    }
    if (!machineId) {
      toast({
        title: t("common.error"),
        description: t("mixing.messages.selectMachine"),
        variant: "destructive",
      });
      return;
    }
    if (materials.length === 0) {
      toast({
        title: t("common.error"),
        description: t("mixing.messages.addOneMaterial"),
        variant: "destructive",
      });
      return;
    }
    if (
      ...
      toast({
        title: t("common.error"),
        description: t("mixing.messages.checkMaterialData"),
        variant: "destructive",
      });
      return;
    }
                ...
                {ordersLoading ? (
                  ...
                  >
                    <SelectTrigger data-testid="select-production-order">
                      <SelectValue
                        placeholder={t("mixing.selectProductionOrder")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <div className="col-span-5 space-y-1">
                        ...
                        {itemsLoading ? (
                          ...
                          >
                            ...
                            >
                              <SelectValue
                                placeholder={t("mixing.selectMaterial")}
                              />
                            </SelectTrigger>
                            <SelectContent>
        ...
        <Card>
              ...
              <Button
                ...
                onClick={() => {
                  const printArea = document.getElementById(
                    "mixing-batches-print-area-page",
                  );
                  if (!printArea) return;
          ...
          <CardContent>
            {batchesLoading ? (
              ...
              <>
                <div
                  id="mixing-batches-print-area-page"
                  style={{ display: "none" }}
                >
                                  ...
                                  <>
                                    {screwBatches.map((batch, index) => {
                                      ...
                                      return (
                                        <TableRow
                                          key={batch.id}
                                          className={`${hoverBg} cursor-pointer text-xs ${index % 2 === 0 ? "" : "bg-gray-50/50"}`}
        ...
        <DialogContent className="max-w-2xl" dir="rtl">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">
              {t("mixing.batchDetailsDescription")}
            </DialogDescription>
          </DialogHeader>
          {selectedBatch && (
            <div className="space-y-4">
              ...
              {selectedBatch.ingredients &&
                selectedBatch.ingredients.length > 0 && (
                  <div className="space-y-2">
                    <Label className="text-lg font-semibold">
                      {t("mixing.ingredients")}
                    </Label>
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead className="text-right">
                            {t("mixing.ingredientName")}
                          </TableHead>
                          <TableHead className="text-right">
                            {t("mixing.weightKg")}
                          </TableHead>
