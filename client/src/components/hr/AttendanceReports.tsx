...
export default function AttendanceReports() {
  ...
  const totals = filteredData.reduce(
    (acc, emp) => ({
      ...
      overtimeHours:
        acc.overtimeHours + (parseFloat(emp.total_overtime_hours as any) || 0),
      lateMinutes:
        acc.lateMinutes + (parseInt(emp.total_late_minutes as any) || 0),
    }),
    {
      presentDays: 0,
      absentDays: 0,
  ...
  const { data: employeeReportData, isLoading: isLoadingEmployeeReport } =
    useQuery<{ success: boolean; report: EmployeeReportData }>({
      queryKey: [
      ...
      case "حاضر":
        return (
          <Badge className="bg-green-100 text-green-800">
            {t("hr.reports.statusPresent")}
          </Badge>
        );
      case "غائب":
        return (
          <Badge className="bg-red-100 text-red-800">
            {t("hr.reports.statusAbsent")}
          </Badge>
        );
      case "إجازة":
        return (
          <Badge className="bg-blue-100 text-blue-800">
            {t("hr.reports.statusLeave")}
          </Badge>
        );
      case "مغادر":
        return (
          <Badge className="bg-yellow-100 text-yellow-800">
            {t("hr.reports.statusLeft")}
          </Badge>
        );
      default:
              ...
              <Select value={selectedPeriod} onValueChange={handlePeriodChange}>
                ...
                <SelectContent>
                  <SelectItem value="week">
                    {t("hr.reports.thisWeek")}
                  </SelectItem>
                  <SelectItem value="month">
                    {t("hr.reports.thisMonth")}
                  </SelectItem>
                  <SelectItem value="custom">
                    {t("hr.reports.customPeriod")}
                  </SelectItem>
                </SelectContent>
              </Select>
        ...
        <CardContent>
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="text-sm text-gray-500">
              {t("hr.reports.periodLabel")}: {format(startDate, "dd/MM/yyyy")} -{" "}
                ...
                <TableHeader>
                  <TableRow className="bg-gray-50 dark:bg-gray-800">
                    <TableHead className="text-right min-w-[150px]">
                      {t("hr.reports.employee")}
                    </TableHead>
                    <TableHead className="text-right min-w-[100px]">
                      {t("hr.reports.jobTitle")}
                    </TableHead>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    filteredData.map((employee) => {
                      const presentDays =
                        parseInt(employee.present_days as any) || 0;
                      const absentDays =
                        parseInt(employee.absent_days as any) || 0;
                      const lateDays = parseInt(employee.late_days as any) || 0;
                      const workHours =
                        parseFloat(employee.total_work_hours as any) || 0;
                      const overtimeHours =
                        parseFloat(employee.total_overtime_hours as any) || 0;
                      const lateMinutes =
                        parseInt(employee.total_late_minutes as any) || 0;
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="text-center">
                            <Badge
                              variant="outline"
                              className={`${absentDays > 0 ? "bg-red-50 text-red-700 border-red-200" : "bg-gray-50 text-gray-600"}`}
                            >
                          ...
                          <TableCell className="text-center">
                            <Badge
                              variant="outline"
                              className={`${lateDays > 0 ? "bg-yellow-50 text-yellow-700 border-yellow-200" : "bg-gray-50 text-gray-600"}`}
                            >
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 text-sm">
              <div className="flex items-center gap-4">
                <span className="font-semibold">
                  {t("hr.reports.periodTotal")}:
                </span>
                <span className="text-green-600">
                  {t("hr.reports.attendance")}: {totals.presentDays}{" "}
                  {t("hr.reports.day")}
                </span>
                <span className="text-red-600">
                  {t("hr.reports.absence")}: {totals.absentDays}{" "}
                  {t("hr.reports.day")}
                </span>
                <span className="text-yellow-600">
                  {t("hr.reports.tardiness")}: {totals.lateDays}{" "}
                  {t("hr.reports.day")}
                </span>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-blue-600">
                  {t("hr.reports.workHoursCol")}:{" "}
                  {formatHours(totals.workHours)}
                </span>
                <span className="text-purple-600">
                  {t("hr.reports.overtimeCol")}:{" "}
                  {formatHours(totals.overtimeHours)}
                </span>
              </div>
            </div>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">
              {t("hr.reports.detailedReportDesc")}
            </DialogDescription>
          </DialogHeader>

          {isLoadingEmployeeReport ? (
            <div className="text-center py-8">
              {t("hr.reports.loadingReport")}
            </div>
          ) : report ? (
            <div className="space-y-4">
                ...
                <div className="border rounded-lg p-6 bg-white">
                  <div className="header text-center mb-6">
                    <div className="flex items-center justify-center gap-3 mb-2">
                      ...
                      <h1 className="text-2xl font-bold">
                        {report.factoryInfo.name}
                      </h1>
                    </div>
                    <p className="text-gray-600">
                      {report.factoryInfo.address}
                    </p>
                    <p className="text-gray-500 text-sm">
                      {report.factoryInfo.phone} | {report.factoryInfo.email}
                    </p>
                  </div>

                  <Separator className="my-4" />
                  <h2 className="text-xl font-semibold text-center mb-4">
                    {t("hr.reports.employeeAttendanceReport")}
                  </h2>
                  ...
                  <div className="grid grid-cols-2 gap-6 mb-6">
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="font-semibold">
                          {t("hr.reports.employeeData")}
                        </span>
                      </div>
                      <div className="space-y-1 text-sm">
                        <p>
                          <strong>{t("hr.reports.name")}:</strong>{" "}
                          {report.employee.name}
                        </p>
                        <p>
                          <strong>{t("hr.reports.employeeNumber")}:</strong>{" "}
                          {report.employee.employeeNumber}
                        </p>
                        <p>
                          <strong>{t("hr.reports.department")}:</strong>{" "}
                          {report.employee.department}
                        </p>
                        <p>
                          <strong>{t("hr.reports.position")}:</strong>{" "}
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="font-semibold">
                          {t("hr.reports.timePeriod")}
                        </span>
                      </div>
                      <div className="space-y-1 text-sm">
                        <p>
                          <strong>{t("hr.reports.from")}:</strong>{" "}
                          {report.period.startDate}
                        </p>
                        <p>
                          <strong>{t("hr.reports.to")}:</strong>{" "}
                          {report.period.endDate}
                        </p>
                        <p>
                          <strong>{t("hr.reports.totalDays")}:</strong>{" "}
                  ...
                  <div className="summary grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div className="summary-item p-3 bg-green-50 rounded-lg text-center">
                      ...
                      <p className="text-sm text-gray-600">
                        {t("hr.reports.presentDays")}
                      </p>
                    </div>
                    <div className="summary-item p-3 bg-red-50 rounded-lg text-center">
                      <p className="text-2xl font-bold text-red-600">
                        {report.summary.absentDays}
                      </p>
                      <p className="text-sm text-gray-600">
                        {t("hr.reports.absentDays")}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div className="p-3 bg-orange-50 rounded-lg text-center">
                      ...
                      <p className="text-sm text-gray-600">
                        {t("hr.reports.overtimeHoursLabel")}
                      </p>
                    </div>
                  </div>

                  <h3 className="font-semibold mb-3">
                    {t("hr.reports.attendanceDetails")}
                  </h3>
                  <div className="border rounded-lg overflow-hidden">
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-gray-100">
                          ...
                          <TableHead className="text-right">
                            {t("hr.reports.dayName")}
                          </TableHead>
                          <TableHead className="text-center">
                            {t("hr.reports.status")}
                          </TableHead>
                          <TableHead className="text-center">
                            {t("hr.reports.checkIn")}
                          </TableHead>
                          <TableHead className="text-center">
                            {t("hr.reports.checkOut")}
                          </TableHead>
                          <TableHead className="text-center">
                            {t("hr.reports.workHoursCol")}
                          </TableHead>
                  ...
                  <div className="mt-4 text-center text-xs text-gray-400">
                    <p>
                      {t("hr.reports.reportGeneratedAt")}:{" "}
