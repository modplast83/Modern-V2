...
export default function SMSSettingsTab() {
  ...
  const { data: smsStatus, isLoading: statusLoading } =
    useQuery<SMSStatusResponse>({
  ...
  } = useQuery<SMSBalanceResponse>({
    queryKey: ["/api/sms/balance"],
    enabled: !!smsStatus?.configured,
  });

  const { data: sendersData, isLoading: sendersLoading } =
    useQuery<SMSSendersResponse>({
  ...
  const addRecipient = () => {
    if (
      newRecipient.trim() &&
      !smsForm.recipients.includes(newRecipient.trim())
    ) {
      setSmsForm((prev) => ({
        ...prev,
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-3 p-4 rounded-lg border">
              <div
                className={`p-2 rounded-full ${smsStatus?.configured ? "bg-green-100" : "bg-red-100"}`}
              >
                {statusLoading ? (
                        ...
                        label =
                          typeof sender.senderName === "string"
                            ...
                            : typeof sender.name === "string"
                              ? sender.name
                              : typeof sender.destination === "string"
                                ? sender.destination
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <div className="flex justify-between mt-1 text-xs text-muted-foreground">
              <span>{t("sms.smsCount", { count: smsCount })}</span>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setSmsForm((prev) => ({ ...prev, sender_name: e.target.value }))
              }
