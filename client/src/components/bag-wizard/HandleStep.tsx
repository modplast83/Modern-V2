    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {allowed.map((handle) => {
          ...
          const isDisabled =
            handleRule &&
            ((handleRule.min_width &&
              config.width > 0 &&
              config.width < handleRule.min_width) ||
              (handleRule.min_thickness &&
                config.thickness > 0 &&
                config.thickness < handleRule.min_thickness));
              ...
              <div>
                ...
                {isDisabled && handleRule && (
                  <div className="text-xs text-red-500 mt-1">
                    {handleRule.min_width &&
                      config.width < handleRule.min_width &&
                      `يتطلب عرض ${handleRule.min_width}+ سم`}
                    {handleRule.min_thickness &&
                      config.thickness < handleRule.min_thickness &&
                      ` | يتطلب سماكة ${handleRule.min_thickness}+ ميكرون`}
                  </div>
                )}
              </div>
            </button>
      ...
      {allowed.length === 1 && (
        <p className="text-sm text-gray-400 mt-3">
          هذا النوع من الأكياس يدعم نوع مقبض واحد فقط
        </p>
      )}
    </div>
  );
