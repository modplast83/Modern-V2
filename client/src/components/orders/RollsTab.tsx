...
export default function RollsTab({
  ...
  const filteredRolls = useMemo(() => {
    return rolls.filter((roll) => {
      ...
      const matchesCustomer =
        customerFilter === "all" || roll.customer_id === customerFilter;
  ...
  const stats = useMemo(() => {
    ...
    const totalWeight = filteredRolls.reduce(
  ...
  const getStageConfig = (stage: string) => {
    ...
    > = {
      film: {
        ...
        label: t("orders.rolls.stages.film"),
      ...
      printing: {
        ...
        label: t("orders.rolls.stages.printing"),
      ...
      cutting: {
        ...
        label: t("orders.rolls.stages.cutting"),
        ...
        label: t("orders.rolls.stages.done"),
      ...
      archived: {
        ...
        label: t("orders.rolls.stages.archived"),
  ...
  const exportToExcel = async () => {
    ...
    worksheet.columns = [
      ...
      {
        header: t("orders.rolls.productionOrderNumber"),
        key: "productionOrder",
        width: 20,
      },
    ...
    filteredRolls.forEach((roll) => {
      worksheet.addRow({
        ...
        waste: roll.waste_kg || "-",
        createdAt: format(new Date(roll.created_at), "yyyy-MM-dd HH:mm", {
          locale: ar,
        }),
      });
    });
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {t("orders.rolls.stages.printing")}
                  </span>
                </div>
                <span className="text-2xl font-bold text-purple-600">
                  {stats.byStage.printing}
                </span>
              </div>
              <Progress
                value={stats.stageProgress.printing}
                className="h-1.5"
              />
            </div>
          </CardContent>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {t("orders.rolls.stages.cutting")}
                  </span>
                </div>
                <span className="text-2xl font-bold text-orange-600">
                  {stats.byStage.cutting}
                </span>
              </div>
        ...
        <Card className="border-r-4 border-r-amber-500">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">
                  {t("orders.rolls.totalWeight")}
                </p>
                <p className="text-xl font-bold text-amber-600">
                  {stats.totalWeight.toLocaleString("en-US")} {t("common.kg")}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 p-4 bg-muted/50 rounded-lg animate-in slide-in-from-top-2">
                  ...
                  <Select value={stageFilter} onValueChange={setStageFilter}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="archived">
                        {t("orders.rolls.stages.archived")}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("orders.rolls.allCustomers")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">
                        {t("orders.rolls.allCustomers")}
                      </SelectItem>
          ...
          <div className="flex items-center justify-between">
            ...
            {selectedRolls.size > 0 && (
              <Badge variant="default">
                {selectedRolls.size} {t("orders.rolls.selected")}
              </Badge>
            )}
          </div>
        </CardHeader>
          ...
          {isRollsLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-lg font-medium text-muted-foreground">
                {t("orders.rolls.noRolls")}
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                {t("orders.rolls.tryChangingFilters")}
              </p>
            </div>
          ) : (
                ...
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    <TableHead className="w-12 text-center">
                      <Checkbox
                        checked={
                          selectedRolls.size === filteredRolls.length &&
                          filteredRolls.length > 0
                        }
                    ...
                    <TableHead className="font-semibold">
                      {t("orders.rolls.rollNumber")}
                    </TableHead>
                    <TableHead className="font-semibold text-center">
                      {t("orders.rolls.stage")}
                    </TableHead>
                    <TableHead className="font-semibold">
                      {t("orders.rolls.productionOrder")}
                    </TableHead>
                    <TableHead className="font-semibold">
                      {t("orders.customer")}
                    </TableHead>
                    <TableHead className="font-semibold">
                      {t("orders.rolls.product")}
                    </TableHead>
                    <TableHead className="font-semibold text-center">
                      {t("orders.rolls.weight")}
                    </TableHead>
                    <TableHead className="font-semibold text-center">
                      {t("orders.rolls.date")}
                    </TableHead>
                    <TableHead className="font-semibold text-center w-20">
                      {t("orders.rolls.actions")}
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    return (
                      <Fragment key={roll.roll_id}>
                        <TableRow
                          ...
                          onClick={() =>
                            setExpandedRow(isExpanded ? null : roll.roll_id)
                          }
                        >
                          <TableCell
                          ...
                          >
                            <Checkbox
                              ...
                              onCheckedChange={() =>
                                toggleRollSelection(roll.roll_id)
                              }
                            />
                          </TableCell>
                          ...
                          <TableCell>
                            <div className="space-y-0.5">
                              <div className="text-sm">
                                {ln(roll.item_name_ar, roll.item_name) || "-"}
                              </div>
                              {roll.size_caption && (
                            ...
                            <TableCell colSpan={9} className="p-4">
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                ...
                                {roll.waste_kg && (
                                  <div>
                                    <span className="text-muted-foreground">
                                      {t("orders.rolls.waste")}:
                                    </span>
                                    <span className="font-medium mr-2 text-red-600">
                                      {roll.waste_kg} {t("common.kg")}
                                    </span>
                                  </div>
                                )}
                              </div>
      ...
      {!isRollsLoading && filteredRolls.length > 0 && (
        <div className="text-sm text-muted-foreground text-center">
          {t("orders.rolls.showing", {
            ...
            total: rolls.length.toLocaleString("en-US"),
          })}
        </div>
      )}
    </div>
