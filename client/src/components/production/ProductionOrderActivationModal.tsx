    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {isUpdating
              ? t("production.activation.updateAssignment")
              : t("production.activation.activateOrder")}
          </DialogTitle>
          <DialogDescription className="sr-only">
            {t("production.activation.assignMachineAndOperator")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
              ...
              <div className="text-sm space-y-1">
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    {t("production.activation.productionOrderNumber")}:
                  </span>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    {t("production.activation.customer")}:
                  </span>
                  <span className="font-medium">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    {t("production.activation.product")}:
                  </span>
                  <span className="font-medium">{order.size_caption}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    {t("production.activation.quantity")}:
                  </span>
                  <span className="font-medium">
                    {order.quantity_kg} {t("production.units.kg")}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div>
            <Label htmlFor="machine">
              {t("production.activation.machineOptional")}
            </Label>
            <Select
            ...
            >
              <SelectTrigger id="machine" data-testid="select-machine">
                <SelectValue
                  placeholder={t("production.activation.selectMachine")}
                />
              </SelectTrigger>
              <SelectContent>
          ...
          <div>
            <Label htmlFor="operator">
              {t("production.activation.operatorOptional")}
            </Label>
            <Select
            ...
            >
              <SelectTrigger id="operator" data-testid="select-operator">
                <SelectValue
                  placeholder={t("production.activation.selectOperator")}
                />
              </SelectTrigger>
              <SelectContent>
          ...
          {!isUpdating && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
              <p className="text-sm text-yellow-800">
                <strong>{t("production.activation.note")}:</strong>{" "}
                {t("production.activation.activationNote")}
              </p>
            </div>
          )}
        </div>
