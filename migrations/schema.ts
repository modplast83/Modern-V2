...
export const factorySnapshots = pgTable(
  "factory_snapshots",
  {
  ...
  (table) => [unique("factory_snapshots_share_token_key").on(table.shareToken)],
);
...
export const erpIntegrationSettings = pgTable(
...
export const items = pgTable(
  ...
  (table) => [
    ...
    index("idx_items_name_gin").using(
      "gin",
      sql`(((name)::text || ' '::text) || (COALESCE(name_ar`,
    ),
...
export const consumableParts = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(status)::text = ANY (ARRAY[('active'::character varying)::text, ('inactive'::character varying)::text])`,
    ),
...
export const customers = pgTable(
  ...
  (table) => [
    ...
    index("idx_customers_name_gin").using(
      "gin",
      sql`(((name)::text || ' '::text) || (COALESCE(name_ar`,
    ),
...
export const erpSyncSchedules = pgTable(
  "erp_sync_schedules",
  {
    id: serial().primaryKey().notNull(),
    erpConfigId: integer("erp_config_id"),
